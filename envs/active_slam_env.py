...
Fixes applied (v2):
- FIX 1: COV_MAX_FOR_NORM updated to 4.0 for 6-DOF covariance trace
- FIX 2: visited_map persists across episodes (was wiped, breaking revisit penalty)
- FIX 3: STALE_DATA_THRESHOLD moved to module constant (was dead local variable)
- FIX 4: Frontier direction scalar is now relative to drone yaw (was world-frame)
- FIX 5: Grid/grid_info accessed via thread-safe snapshot from slam_collector
- FIX 6: Occupancy channel uses max-pooling in resize (was mean, created phantom values)
- FIX 7: Step docstring corrected (said "retreat to origin", code hovers in place)
"""
...
# --- Map & CNN ---
MAP_SIZE = 48              # CNN input: MAP_SIZE x MAP_SIZE.
                           # Must match lightweight_train/env_3d.py GRID_SIZE:
                           # the transferred policy's CNN feature extractor was
                           # trained with (MAP_CHANNELS, 48, 48) input, so the
                           # Gazebo env must produce the same shape.

# --- Flight ---
FLIGHT_ALTITUDE = -ALTITUDE_LEVELS[0]
...
# --- Altitude management ---
ENABLE_ALTITUDE_MGMT = True
ALT_STEP = ALTITUDE_LEVELS[1] - ALTITUDE_LEVELS[0]
ALT_MIN = ALTITUDE_LEVELS[0]
ALT_MAX = ALTITUDE_LEVELS[-1]
FRONTIER_ZERO_THRESHOLD = 10
MIN_ALT_DWELL = 20         # Min steps between altitude switches.
                           # Mirrors lightweight_train/env_3d.py — the transferred
                           # policy was trained against this dwell absorber.
                           # Without it, the policy's dz-vote rate (~65-80% of
                           # steps) would trigger Z-filter reconfig every step,
                           # corrupting RTAB-Map visual odometry.
MAX_ALT_IDX = 2            # Hard cap on altitude index (0=1m, 1=2m, 2=3m, 3=4m).
                           # Prior dry run showed RTAB-Map losing tracking for 331s
                           # at alt=4m where the warehouse ceiling is feature-sparse.
                           # Limiting to alt_idx<=2 keeps the camera in the shelf-
                           # dense zone (1–3m) where visual odometry has features.
...
COV_MAX_FOR_NORM = 4.0     # Covariance normalization ceiling
                           # Was 2.0 for XYZ-only trace; now 4.0 because
                           # slam_collector FIX 5 returns full 6-DOF trace
                           # (includes roll/pitch/yaw uncertainty)
...
STALE_DATA_THRESHOLD = 8   # If covariance frozen for 8 steps, declare silent SLAM failure

W_ALT_SWITCH_GOOD = 1.0
W_ALT_IMBALANCE = -0.2
ALT_IMBALANCE_THRESH = 0.10
...
class ActiveSLAMEnv(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        self.observation_space = spaces.Dict({
            "map_tensor": spaces.Box(
                low=0.0, high=1.0,
                shape=(MAP_CHANNELS, MAP_SIZE, MAP_SIZE),
                dtype=np.float32,
            ),
            "scalars": spaces.Box(
                low=0.0, high=1.0,
        ...
        self.alt_idx = 0
        self.altitude_changes = 0
        self.zero_frontier_steps = 0
        self.steps_at_current_alt = 0

        # Transfer-compatible per-altitude memory
        self.altitude_occupancy_maps = np.full((N_ALTITUDES, MAP_SIZE, MAP_SIZE), 0.5, dtype=np.float32)
        self.visited_maps = np.zeros((N_ALTITUDES, MAP_SIZE, MAP_SIZE), dtype=np.float32)
        self.trajectory_maps = np.zeros((N_ALTITUDES, MAP_SIZE, MAP_SIZE), dtype=np.float32)
        self.alt_known_cells = np.zeros(N_ALTITUDES, dtype=np.float32)
        self.alt_total_cells = np.zeros(N_ALTITUDES, dtype=np.float32)
    ...
    def _altitude_to_index(self, altitude_m):
        diffs = [abs(level - altitude_m) for level in ALTITUDE_LEVELS]
        return int(np.argmin(diffs))

    def _set_altitude_level(self, alt_idx):
        alt_idx = int(np.clip(alt_idx, 0, N_ALTITUDES - 1))
        if alt_idx == self.alt_idx:
            return False
        self.alt_idx = alt_idx
        self.current_flight_altitude = ALTITUDE_LEVELS[self.alt_idx]
        self.altitude_changes += 1
        if ENABLE_Z_FILTER:
            self._apply_z_filter(self.current_flight_altitude)
        return True

    def _alt_cov(self, alt_idx):
        total = max(float(self.alt_total_cells[alt_idx]), 1.0)
        return float(self.alt_known_cells[alt_idx]) / total

    def _total_cov(self):
        total = max(float(np.sum(self.alt_total_cells)), 1.0)
        return float(np.sum(self.alt_known_cells)) / total

    def _update_altitude_buffers(self, occ_map, visited_map, traj_map, known_cells, total_cells):
        self.altitude_occupancy_maps[self.alt_idx] = occ_map
        self.visited_maps[self.alt_idx] = visited_map
        self.trajectory_maps[self.alt_idx] = traj_map
        if np.all(self.alt_total_cells == 0):
            self.alt_total_cells[:] = max(total_cells, 1)
        self.alt_known_cells[self.alt_idx] = float(known_cells)
        self.alt_total_cells[self.alt_idx] = float(max(total_cells, 1))

    # ================================================================
    # Observation Building
    # ================================================================
    def _get_observation(self):
        # FIX 5: Get a consistent (grid, grid_info) snapshot under lock.
        # Prevents torn reads where grid_info is from a newer callback
        # than occupancy_grid, causing wrong coordinate transforms.
        grid, grid_info, known_cells, total_cells = self.ros_node.get_grid_snapshot()
        if grid is None:
        ...
        if alt is not None:
            self.alt_idx = self._altitude_to_index(float(alt))
            self.current_flight_altitude = ALTITUDE_LEVELS[self.alt_idx]
        ...
        # FIX 6: Occupancy uses max-pooling (conservative obstacle detection)
        ch1_r = self._resize_channel(ch1, MAP_SIZE, pad_value=0.5, mode='max')
        ch2_r, ch3_r = self._update_altitude_memory(h, w, grid_info=grid_info)
        self._update_altitude_buffers(ch1_r, ch2_r, ch3_r, known_cells, total_cells)
        self._detect_frontiers(grid, grid_info=grid_info)
        alt_coverages = [self._alt_cov(idx) for idx in range(N_ALTITUDES)]
        least_explored_alt = int(np.argmin(alt_coverages))
        visited_across = np.mean(self.visited_maps, axis=0)
        map_tensor = np.stack(
            [
                self.altitude_occupancy_maps[self.alt_idx],
                self.visited_maps[self.alt_idx],
                self.trajectory_maps[self.alt_idx],
                self.altitude_occupancy_maps[least_explored_alt],
                visited_across,
            ],
            axis=0,
        )

        # Scalar features (10 values, all normalized to ~[0, 1])
        ...
        coverage = self._total_cov()
        frontier_dist_norm = min(self.nearest_frontier_dist / 10.0, 1.0)
        # FIX 4: Frontier direction relative to drone heading, not world-frame.
        # Old: absolute atan2 angle — agent can't distinguish "frontier is north"
        # from "frontier is to my left" without also knowing its own heading.
        # New: (frontier_dir - drone_yaw) wrapped to [-π, π], then normalized.
        drone_yaw = self.ros_node.get_drone_yaw()
        relative_frontier_dir = self.nearest_frontier_dir - drone_yaw
        # Wrap to [-π, π]
        relative_frontier_dir = (relative_frontier_dir + np.pi) % (2 * np.pi) - np.pi
        frontier_dir_norm = (relative_frontier_dir + np.pi) / (2 * np.pi)
        altitude_norm = self.alt_idx / max(N_ALTITUDES - 1, 1)
        step_frac = self.step_count / MAX_STEPS
        current_alt_cov = alt_coverages[self.alt_idx]
        other_cov = [cov for idx, cov in enumerate(alt_coverages) if idx != self.alt_idx]
        other_potential = max((1.0 - cov) for cov in other_cov) if other_cov else 0.0
        min_other_cov = min(other_cov) if other_cov else 1.0

        scalars = np.array([
            cov_norm, frontier_count_norm, coverage,
            frontier_dist_norm, frontier_dir_norm,
            altitude_norm, current_alt_cov, other_potential, min_other_cov, step_frac,
        ], dtype=np.float32)

        return {"map_tensor": map_tensor, "scalars": scalars}

    def _update_altitude_memory(self, h, w, grid_info=None):
        visited_map = self.visited_maps[self.alt_idx].copy()
        trajectory_map = self.trajectory_maps[self.alt_idx] * 0.92
        pos = self.ros_node.get_drone_position()
        gi = grid_info if grid_info is not None else self.ros_node.grid_info
        if pos is not None and gi is not None:
            max_dim = max(h, w)
            scale = MAP_SIZE / max(max_dim, 1)
            gx = int((pos[0] - gi.origin.position.x) / gi.resolution)
            gy = int((pos[1] - gi.origin.position.y) / gi.resolution)
            mx = int(np.clip(gx * scale, 0, MAP_SIZE - 1))
            my = int(np.clip(gy * scale, 0, MAP_SIZE - 1))
            for dx in range(-VISIT_RADIUS, VISIT_RADIUS + 1):
                for dy in range(-VISIT_RADIUS, VISIT_RADIUS + 1):
                    nx, ny = mx + dx, my + dy
                    if 0 <= nx < MAP_SIZE and 0 <= ny < MAP_SIZE:
                        visited_map[ny, nx] = 1.0
            trajectory_map[my, mx] = 1.0
        self.visited_maps[self.alt_idx] = visited_map
        self.trajectory_maps[self.alt_idx] = trajectory_map
        return visited_map, trajectory_map

    def _resize_channel(self, channel, target_size, pad_value=0.5, mode='mean'):
        """Resize a 2D channel to target_size × target_size.

        FIX 6: Added mode parameter. For occupancy (ch1), 'max' pooling is
        more appropriate than 'mean' — mean-averaging 0.0/0.5/1.0 produces
        phantom values like 0.25 that don't correspond to any real occupancy
        state.  Max pooling is conservative: if ANY cell in the block is
        occupied (1.0), the output is occupied.
        """
        h, w = channel.shape
        ...
        if max_dim >= target_size:
            ...
            if mode == 'max':
                resized = resized.max(axis=(1, 3))
            else:
                resized = resized.mean(axis=(1, 3))
        else:
    ...
    def _empty_observation(self):
        return {
            "map_tensor": np.full((MAP_CHANNELS, MAP_SIZE, MAP_SIZE), 0.0, dtype=np.float32),
    ...
    # ================================================================
    def _detect_frontiers(self, grid, grid_info=None):
        h, w = grid.shape
        unknown_mask = (grid == -1)
        # RTAB-Map's Z-sliced 2D projection marks a cell occupied if any voxel
        # in the Z-column is occupied, so in a warehouse almost every observed
        # cell ends up occupied (diagnostic: 1173 occ vs 7 free at step 100).
        # Classical free∧adj_unknown yields zero frontiers.
        # Use "unknown cell adjacent to any known cell" — the edge of the
        # observed region, regardless of whether the known side is free or
        # occupied. Still a valid active-SLAM exploration signal.
        known_mask = ~unknown_mask

        known_adjacent = np.zeros_like(unknown_mask)
        known_adjacent[1:, :] |= known_mask[:-1, :]
        known_adjacent[:-1, :] |= known_mask[1:, :]
        known_adjacent[:, 1:] |= known_mask[:, :-1]
        known_adjacent[:, :-1] |= known_mask[:, 1:]

        frontier_mask = unknown_mask & known_adjacent
        ...
        for y in range(h):
            for x in range(w):
                if frontier_mask[y, x] and labels[y, x] == 0:
                    ...
                    # Min cluster size 8 ≈ 80cm of boundary at ~0.1m grid
                    # resolution. Filters out sensor-shadow holes around the
                    # drone that would otherwise register as "nearest frontier
                    # at 0m" and confuse the policy's direction signal.
                    if len(cells) >= 8:
    ...
    def _get_revisit_ratio(self):
        ...
        gi = self.ros_node.grid_info
        grid, _grid_info, _known, _total = self.ros_node.get_grid_snapshot()
        if grid is None:
            return 0.0
        max_dim = max(grid.shape[0], grid.shape[1], 1)
        scale = MAP_SIZE / max_dim
        gx = int((pos[0] - gi.origin.position.x) / gi.resolution * scale)
        gy = int((pos[1] - gi.origin.position.y) / gi.resolution * scale)
        visited_map = self.visited_maps[self.alt_idx]
    ...
    def reset(self, seed=None, options=None):
        ...
        # FIX 2: Do NOT reset visited_map to None on episode 2+.
        # The SLAM map persists across episodes (by design), so the revisit
        # penalty must also persist — otherwise the agent isn't penalized for
        # re-covering episode 1 areas in episode 2, creating inconsistent
        # reward signals.  Only initialize on very first episode.
        # (self.visited_map starts as None in __init__, and _update_visited_map
        # will create it on first call.)
        ...
        info = {
            ...
            "initial_coverage": self._total_cov(),
    ...
    def _compute_reward(self, action, collision_occurred=False):
        ...
        alt_coverages = [self._alt_cov(idx) for idx in range(N_ALTITUDES)]
        current_alt_cov = alt_coverages[self.alt_idx]
        min_alt_cov = min(alt_coverages)
        altitude_reward = 0.0
        if len(action) >= 3 and abs(float(action[2])) > 0.2:
            altitude_reward += W_ALT_SWITCH_GOOD * max(0.0, min_alt_cov - current_alt_cov)
        imbalance_gap = current_alt_cov - min_alt_cov
        if imbalance_gap > ALT_IMBALANCE_THRESH:
            altitude_reward += W_ALT_IMBALANCE * imbalance_gap
        reward += altitude_reward
        info["r_altitude"] = altitude_reward
        ...
        for idx, cov in enumerate(alt_coverages):
            info[f"cov_alt{idx}"] = cov
    ...
    def _build_common_info(self):
        ...
        return {
            ...
            "known_cells":           int(np.sum(self.alt_known_cells)),
            "total_cells":           int(np.sum(self.alt_total_cells)),
            "coverage":              coverage,
            ...
            "altitude":              self.current_flight_altitude if alt is None else float(alt),
            "alt_idx":               self.alt_idx,
            "altitude_changes":      self.altitude_changes,
            ...
            **{f"cov_alt{idx}": cov for idx, cov in enumerate(alt_coverages)},
        }
    ...
    def step(self, action):
        ...
        - If RTAB-Map tracking lost → HOVER IN PLACE (do NOT fly to origin)
        - If tracking lost > 30 steps → force episode truncation
        - If collision → retreat to last good pos; > 15 consecutive → truncation
        - Yaw rate-limited to ~11°/step
        ...
        alt_changed = False
        if ENABLE_ALTITUDE_MGMT and len(action) >= 3:
            wants_up = float(action[2]) > ALT_UP_THRESH and self.alt_idx < min(MAX_ALT_IDX, N_ALTITUDES - 1)
            wants_down = float(action[2]) < ALT_DOWN_THRESH and self.alt_idx > 0
            if (wants_up or wants_down) and self.steps_at_current_alt >= MIN_ALT_DWELL:
                if wants_up:
                    alt_changed = self._set_altitude_level(self.alt_idx + 1)
                else:
                    alt_changed = self._set_altitude_level(self.alt_idx - 1)
        if alt_changed:
            self.steps_at_current_alt = 0
        ...
        if self._stale_data_steps >= STALE_DATA_THRESHOLD and not self.ros_node.is_tracking_lost():
        ...
        # ----------------------------------------------------------
        if not terminated and not truncated and ENABLE_ALTITUDE_MGMT:
            self.current_flight_altitude = ALTITUDE_LEVELS[self.alt_idx]
